...
if [ -z "$1" ] || [ $(stat -f "%m" ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl/vms) -ge $TIME ]
   then
   # cache the VMs
    if [ ! -d ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl/ ]
        then
        mkdir ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl
    fi
    VBoxManage list vms -l | sed -n -e '/^Name/N' -e '/\nGroups/P' -e '/\nGuest OS/p' -e '/^Guest OS/P' -e '/^UUID/P' -e '/^State/P' > ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl/vms
fi
...
if [[ $(echo "$1" | grep -c " >" ) -gt 0 ]]
    ...
    VMINFO=$(grep -A 3 "$NAME$" ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl/vms)
    if [[ ! -z "$VMINFO" ]]
        then
...
if [ ! -z "$UUID" ] && [ $(echo $VMINFO | egrep -c " running ") -gt 0 ]
    ...
    done < ~/Library/Caches/com.runningwithcrayons.Alfred/Workflow\ Data/dk.aiyo.VirtualBoxControl/vms
fi
