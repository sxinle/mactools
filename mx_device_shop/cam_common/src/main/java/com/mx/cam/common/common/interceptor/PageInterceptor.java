...
public class PageInterceptor implements Interceptor {  
    ...
    public Object intercept(Invocation invocation) throws Throwable {  
        ...
        if (obj instanceof SearchPageUtil) {
            ...
            if(page.getPage()!=null && page.getRows()!=null) {
                //if (!StringUtil.isEmpty(page.getPage().toString()) && !StringUtil.isEmpty(page.getRows().toString())) {
    	            ...
    	            if(sql.toLowerCase().startsWith("select")) {
    		            ...
    		            ReflectUtil.setFieldValue(boundSql, "sql", pageSql.replace(";", ""));
    	            }
            }
        }
    ...
    private String getMysqlPageSql(SearchPageUtil page, StringBuffer sqlBuffer) {
    	sqlBuffer.insert(0, "select * from (").append(") pageSqlx where 1=1");
    	String sqlSort = page.getSort();
    	String sqlOrder = page.getOrder();
    	if (!StringUtil.isEmpty(sqlSort)) {
    		if (StringUtil.isEmpty(sqlOrder)) {
    			sqlOrder = "";
    		}
    		sqlBuffer.append(" ORDER BY ").append(sqlSort).append(" ").append(sqlOrder);
    	}
    	sqlBuffer.insert(0, "select * from (").append(") pageSqlj");
...
//            return "select count(*) from (select count(*) " + sql.substring(index) + ") pageSqlTotal";  
//
//        }else{
